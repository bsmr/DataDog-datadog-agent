...
func TestProcessEventsType(t *testing.T) {
	...
	// We need to check that the countByAction concatenated string contains the source events.
	// As the order is not guaranteed we want to use contains.
	firstCall := (mocked.Calls[0].Arguments.Get(0)).(metrics.Event).Text
	assert.Contains(t, firstCall, "2 **Scheduled**")
	assert.Contains(t, firstCall, "3 **Started**")

	secondCall := (mocked.Calls[1].Arguments.Get(0)).(metrics.Event).Text
	assert.Contains(t, secondCall, "4 **BackOff**")
